...
var (
	...
	informationSchemaUserStatisticsTypes = map[string]struct {
		...
		"TOTAL_SSL_CONNECTIONS": {prometheus.CounterValue,
			newDesc(informationSchema, "user_statistics_total_ssl_connections_total",
	...
	for informationSchemaUserStatisticsRows.Next() {
		...
		for idx, columnName := range columnNames[1:] {
			if metricType, ok := informationSchemaUserStatisticsTypes[columnName]; ok {
				ch <- mustNewConstMetric(&ctx, metricType.desc, metricType.vtype, float64(userStatData[idx]), user)
			} else {
				// Unknown metric. Report as untyped.
				desc := newDesc(informationSchema, fmt.Sprintf("user_statistics_%s", strings.ToLower(columnName)), fmt.Sprintf("Unsupported metric from column %s", columnName), []string{"user"}, nil)
				ch <- mustNewConstMetric(&ctx, desc, prometheus.UntypedValue, float64(userStatData[idx]), user)
			}
		}
	}
	return nil
