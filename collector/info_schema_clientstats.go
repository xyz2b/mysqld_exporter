...
var (
	...
	informationSchemaClientStatisticsTypes = map[string]struct {
		...
		"TOTAL_SSL_CONNECTIONS": {prometheus.CounterValue,
			newDesc(informationSchema, "client_statistics_total_ssl_connections_total",
		...
		"MAX_STATEMENT_TIME_EXCEEDED": {prometheus.CounterValue,
			newDesc(informationSchema, "client_statistics_max_statement_time_exceeded_total",
	...
	for informationSchemaClientStatisticsRows.Next() {
		...
		for idx, columnName := range columnNames[1:] {
			if metricType, ok := informationSchemaClientStatisticsTypes[columnName]; ok {
				ch <- mustNewConstMetric(&ctx, metricType.desc, metricType.vtype, float64(clientStatData[idx]), client)
			} else {
				// Unknown metric. Report as untyped.
				desc := newDesc(informationSchema, fmt.Sprintf("client_statistics_%s", strings.ToLower(columnName)), fmt.Sprintf("Unsupported metric from column %s", columnName), []string{"client"}, nil)
				ch <- mustNewConstMetric(&ctx, desc, prometheus.UntypedValue, float64(clientStatData[idx]), client)
			}
		}
	}
	return nil
